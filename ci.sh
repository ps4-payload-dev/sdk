...
for SAMPLE in "${MAKE_SAMPLES[@]}"; do
    ${MAKE} -C $PS4_PAYLOAD_SDK/samples/$SAMPLE clean all || exit 1
done

for SAMPLE in "${CMAKE_SAMPLES[@]}"; do
    ${CMAKE} \
	-DCMAKE_VERBOSE_MAKEFILE=YES \
	-B $PS4_PAYLOAD_SDK/build/$SAMPLE \
	-S $SCRIPTDIR/samples/$SAMPLE || exit 1
    ${CMAKE} --build $PS4_PAYLOAD_SDK/build/$SAMPLE
done

for SAMPLE in "${MESON_SAMPLES[@]}"; do
    ${MESON} setup \
	     $SCRIPTDIR/samples/$SAMPLE \
	     $PS4_PAYLOAD_SDK/build/$SAMPLE
    ${MESON} compile -C $PS4_PAYLOAD_SDK/build/$SAMPLE
done
